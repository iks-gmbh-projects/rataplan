...
public class VoteParticipantControllerService {
    ...
    )
    {
        ...
        voteParticipantDTO.setName(voteParticipantDTO.getName().trim());
        VoteParticipant voteParticipant = modelMapper.map(voteParticipantDTO, VoteParticipant.class);
        voteParticipant = voteParticipantService.updateParticipant(vote, oldVoteParticipant, voteParticipant);
    ...
    private void validateAccessToParticipant(VoteParticipant voteParticipant, AuthUser authUser) {
        ...
        if(voteParticipant.getUserId() == null || (
            authUser != null && Objects.equals(authUser.getId(), voteParticipant.getUserId())
        ))
        {
            return;
        }
    ...
    private VoteParticipantDTO createValidDTOParticipant(
        Vote vote, VoteParticipantDTO voteParticipantDTO, AuthUser user
    )
    {
