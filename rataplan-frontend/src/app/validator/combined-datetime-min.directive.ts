...
@Directive({
  ...
  providers: [{provide: NG_VALIDATORS, useExisting: CombinedDatetimeMinDirective, multi: true}],
})
export class CombinedDatetimeMinDirective implements Validator {
  @Input('combinedDatetimeMin') referenceTime: string | AbstractControl | null | undefined | false = null;
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    ...
    if (this.ignoreDate && this.ignoreDate !== 'false') {
      return time < control.value ? null : {matTimepickerMin: true};
    }
    ...
    return Date.parse(self) > Date.parse(reference) ? null : {matTimepickerMin: true};
  }
}
